    ...
    for pid, wps in env.waypoints.items():
        ...
        if len(pts) >= 2:
            for i in range(len(pts) - 1):
                ...
                _pen = QPen(color, 1, Qt.PenStyle.DashLine)
        ...
        for emitter in platform.emitters:
            ...
            if jammer:
                ...
                if jp is not None:
                    ...
                    _pen = QPen(QColor("#9b59b6"), 1.0, Qt.PenStyle.DashLine)
                    _pen.setCosmetic(True)
                    line.setPen(_pen)
                    line.setZValue(2)
                    scene.addItem(line)
                    ...
                    mid.setData(2, ((jx + x) / 2, (jy + y) / 2, 4.0, -4.0))
                    mid.setZValue(3)
                    scene.addItem(mid)
                    screen_fixed(mid)
...
def draw_ir_contacts(scene: QGraphicsScene, env: Environment, proj: LocalProjection,
    ...
    for own_id, c_map in getattr(env, "ir_contacts", {}).items():
        ...
        for contact in c_map.values():
            ...
            label.setData(2, ((x1 + x2) / 2, (y1 + y2) / 2, 0.0, 4.0))
            label.setZValue(4)
            scene.addItem(label)
            screen_fixed(label)
...
def draw_radar_contacts(scene: QGraphicsScene, env: Environment, proj: LocalProjection,
    ...
    for own_id, radar_map in getattr(env, "radar_contacts", {}).items():
        ...
        for contact in radar_map.values():
            ...
            label.setData(2, ((x1 + x2) / 2, (y1 + y2) / 2, 0.0, -6.0))
            label.setZValue(6)
            scene.addItem(label)
            screen_fixed(label)
...
def draw_orders(scene: QGraphicsScene, env: Environment, proj: LocalProjection,
    ...
    for order in env.orders:
        ...
        x2, y2 = proj.to_xy(target.latitude, target.longitude)
        line = QGraphicsLineItem(x1, y1, x2, y2)
        _pen = QPen(QColor("#e74c3c"), 1.0, Qt.PenStyle.DashLine)
        _pen.setCosmetic(True)
        line.setPen(_pen)
        line.setZValue(8)
        scene.addItem(line)
        ...
        label.setData(2, ((x1 + x2) / 2, (y1 + y2) / 2, 0.0, -10.0))
        label.setZValue(9)
        scene.addItem(label)
        screen_fixed(label)
