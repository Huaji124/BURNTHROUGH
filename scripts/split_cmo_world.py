"""将 CMO 世界导出 JSON 按国家/地区拆分，便于本地使用。

生成目录：
    data/cmo_world/<country_slug>/
        country.json
        platforms.json
        sensors.json
        mounts.json
        ...
        comms.json
        fuel.json
    data/cmo_world/index.json

生成的数据是 CMO 商业数据，本地使用；整个目录已加入 .gitignore。
"""
...
DEFAULT_SRC = ROOT / "data" / "cmo_world_full.json"
DEFAULT_OUT = ROOT / "data" / "cmo_world"
...
def main() -> int:
    ...
    for cid, plats in sorted(by_country.items()):
        ...
        save_json(out_dir / "sensors.json",
                  {str(k): v for k, v in all_sensors.items() if int(k) in sensor_ids})
        save_json(out_dir / "mounts.json",
                  {str(k): v for k, v in all_mounts.items() if int(k) in mount_ids})
        save_json(out_dir / "loadouts.json",
                  {str(k): v for k, v in all_loadouts.items() if int(k) in loadout_ids})
        save_json(out_dir / "loadout_weapons.json",
                  [r for r in loadout_weapons if r.get("ID") in loadout_ids])
        save_json(out_dir / "magazines.json",
                  {str(k): v for k, v in all_magazines.items() if int(k) in magazine_ids})
        save_json(out_dir / "magazine_weapons.json",
                  [r for r in magazine_weapons if r.get("ID") in magazine_ids])
        save_json(out_dir / "propulsions.json",
                  {str(k): v for k, v in all_propulsions.items() if int(k) in propulsion_ids})
        save_json(out_dir / "propulsion_performance.json",
                  [r for r in propulsion_performance if r.get("ID") in propulsion_ids])
        save_json(out_dir / "comms.json",
                  {str(k): v for k, v in all_comms.items() if int(k) in comm_ids})
        save_json(out_dir / "fuel.json",
                  {str(k): v for k, v in all_fuel.items() if int(k) in fuel_ids})
        ...
        print(f"  {cname:<40} {len(plats):>5} 平台")
